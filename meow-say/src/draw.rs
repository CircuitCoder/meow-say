use image::{imageops::overlay, GrayImage, ImageBuffer, Luma, Rgb, Rgba};
use imageproc::{
    distance_transform::euclidean_squared_distance_transform,
    drawing::{draw_cubic_bezier_curve_mut, draw_text_mut},
...
pub fn draw(
    ...
    overlay(
        &mut buffer,
        img,
        (width - img_width - 50).into(),
        (height - img_height - 50).into(),
    );
